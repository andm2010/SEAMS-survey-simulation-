...
Mincidence_SE <- function( Incidence, prev,
                           ...
                           prev_error, slope_error, 
                           Covariance_p_s,
                           excess_mortality_error = 0){
  ...
  return(sqrt(
    ...
    + 2*(((1-prev)^(-2) * slope + excess_mortality)^2) * ((1-prev)^(-2)) * Covariance_p_s
...
Midpoint_inc_from_prevalence <- function(N_iterations = 1, T1, T2, excess_mortality_estimate,
  ...
  if (is.vector(pop_prevalence) == TRUE){
    ...
    incidence = ((1 / (1 - midpoint_prevalence)) * prevalence_derivative) + midpoint_prevalence * excess_mortality_estimate(t)
    incidence_se = Mincidence_SE(prev = midpoint_prevalence, slope = prevalence_derivative, prev_error = midpoint_prevalence_se,
                                 slope_error = prevalence_derivative_se, excess_mortality_estimate(t))
    ...
    if(N_iterations == 1){
      ...
      Covariance_p_s = vcov(fit_glm)[1,2]
      ...
      Incidence_PE <- Mincidence_PE(prev = Pred_Midpoint_Prev_PE,
                                    slope = Slope_Exp_PE,
                                    excess_mortality = excess_mortality_estimate(t))

      Incidence_SE <- Mincidence_SE( prev = Pred_Midpoint_Prev_PE,
                                     slope = Slope_Exp_PE, 
                                     Covariance_p_s = 0,
                                     excess_mortality= excess_mortality_estimate(t),
                                     prev_error = Pred_Midpoint_Prev_SE, 
                                     slope_error = Slope_Exp_SE)

      CI_Lower <- Incidence_PE - 1.96 * Incidence_SE
      CI_Upper <- Incidence_PE + 1.96 * Incidence_SE

      results <- data.frame(Method='DM',
                            Slope_PE = Slope_Exp_PE, 
                            Slope_SE = Slope_Exp_SE,
                            Mid_Prev_PE = Pred_Midpoint_Prev_PE, 
                            Mid_Prev_SE = Pred_Midpoint_Prev_SE,
                            Covariance_p_s = Covariance_p_s,
                            Incidence_PE = Incidence_PE,
                            Incidence_SE = Incidence_SE,
                            CI_Upper = CI_Upper, 
                            CI_Lower = CI_Lower)
      ...
      Covariance_p_s = as.vector(rep(NA, N_iterations))
      ...
      for(i in 1:N_iterations) {
        ...
        Covariance_p_s[i] = vcov(fit_glm)[1,2]
        ...
        Incidence_SE[i] <- Mincidence_SE(prev = Pred_Prevalences_PE[i],
                                         slope = Slope_PE[i], 
                                         Covariance_p_s = Covariance_p_s[i],
                                         excess_mortality= excess_mortality_estimate(t),
                                         prev_error = Pred_Prevalences_SE[i], 
                                         slope_error = Slope_SE[i])
      ...
      Boot_summary  <- data.frame(Method='Boot',
                                  ...
                                  Incidence_PE = Boot_Inc_mean,
                                  Incidence_SE = Boot_SE,
                                  Covariance_p_s = Covariance_p_s,
